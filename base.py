...
def main():
   ...
   st.subheader('Top 5 municipios por número de graduados (nacimiento)')
   if 'MUNICIPIO DE NACIMIENTO' in df.columns and grado in df.columns:
       graduates_by_municipio = (df.groupby('MUNICIPIO DE NACIMIENTO')[grado]
                                  .sum()
                                  .sort_values(ascending=False)
                                  .reset_index())
       top_5_mun = graduates_by_municipio.head(5)
       fig_top5 = px.bar(top_5_mun, x='MUNICIPIO DE NACIMIENTO', y=grado,
                         title='Top 5 Municipios de Nacimiento por Número de Graduados',
                         labels={grado: 'Número de Graduados'},
                         color='MUNICIPIO DE NACIMIENTO')
       st.plotly_chart(fig_top5, use_container_width=True)


   st.subheader('Top 5 municipios por participación vs tasa de graduación')
   if 'MUNICIPIO DE NACIMIENTO' in df.columns and grado in df.columns:
       total_participantes_by_municipio = (df.groupby('MUNICIPIO DE NACIMIENTO').size()
       ...
       df_top5_part = df[df['MUNICIPIO DE NACIMIENTO'].isin(top5_part['MUNICIPIO DE NACIMIENTO'])]


       plot_data_comparison = (df_top5_part.groupby('MUNICIPIO DE NACIMIENTO').agg(
           GRADUADOS=(grado, 'sum'),
           TOTAL_PARTICIPANTES=('MUNICIPIO DE NACIMIENTO', 'size'))
           .reset_index())
       plot_data_comparison['PORCENTAJE_GRADUADOS'] = plot_data_comparison['GRADUADOS'] / plot_data_comparison['TOTAL_PARTICIPANTES'] * 100


       fig3 = px.bar(plot_data_comparison, x='MUNICIPIO DE NACIMIENTO', y='PORCENTAJE_GRADUADOS',
